...
for i in range(len(data_v)):
    icrop = preprocess.crop_center(np.squeeze(data_v[i]))
    crop_data_v.append(icrop)
crop_data_v = np.asarray(crop_data_v)
crop_data_v = np.expand_dims(crop_data_v,axis=4)
predictions = model.predict(crop_data_v,batch_size=1,verbose=1)
