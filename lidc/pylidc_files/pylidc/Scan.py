...
class Scan(Base):
    ...
    def load_all_dicom_images(self, verbose=False):
        ...
        if len(sorted_fnames) > len(fnames) :
            sorted_fnames = sorted_fnames[:-1]
        nums_sorted = []
        for i,fname  in enumerate(sorted_fnames):
            nums_sorted.append(int(fname.split('.')[0]))
        min_sorted = np.min(nums_sorted)
        nums_files = []
        for i,fname  in enumerate(fnames):
            nums_files.append(int(fname.split('.')[0]))
        
        min_files = np.min(nums_files)
     
        for i,fname in enumerate(sorted_fnames):
            sorted_fnames[i]=str(int(fname.split('.')[0])+min_files-min_sorted)+'.dcm'
        ...
        zs = [float(img.ImagePositionPatient[2]) for img in images]
        dz = np.diff(zs)
        check = np.min(np.abs(dz-dz[0]))
        if check > 1e-3:
            print 'Error in position spacings'
            exit()
        return images
    ...
    def to_volume(self):
        ...
        zs = [float(img.ImagePositionPatient[2]) for  img in images]
        orientation = [float(x) for x in img.ImageOrientationPatient]
        dz = zs[1]-zs[0]
        spacing = [pixel_spacing,pixel_spacing,dz]
	intercept = float(images[0].RescaleIntercept)	
	slope = float(images[0].RescaleSlope)
	volume = slope*volume + intercept	
	
        return volume,spacing,orientation,zs[0]
