...
BEST_WEIGHTS_PATH = 'logs/best_weights.hdf5' 
...
def get_model(input_shape=(preprocess.SEG_CROP,preprocess.SEG_CROP,preprocess.SEG_CROP,1)):
    ...
    #model.compile(optimizer='adam',loss=resnet3.dice_coef_loss,loss_weights=[0.2,0.3,0.5])
    model.compile(optimizer='adam',loss=binary_crossentropy,loss_weights=[0.2,0.3,0.5])
...
def main():
    ...
    if TRAIN:
        model.fit_generator(generator= train_generator,validation_data=valid_generator,
                nb_val_samples=400,nb_epoch=35,samples_per_epoch=3996,callbacks=[checkpoint,save_best],nb_worker=4,
